<?php
    ...
    function getPalAvg($factors, $sitting_only, $hardly_active, $bit_active, $mainly_standing, $working) {
        ...
        */
        ...
        // check if active hours are over 24 if so it returns NULL to afterwards check if "isset"
        if($activeHours > 24) {
          ...
          // calculate sleep time by our own - the difference between active hours and 24 is our sleeptime which we multiply by our sleep factor
...
//-------------------------------in this block we save every input into variables---------------------------------------
...
//------------------------------calculate caloriedemand based on gender-----------------------------------
        $kalorienBedarf = 0;
        if($gender == "male") {
